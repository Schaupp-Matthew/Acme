...
// Get Thumbnail Information from images table
function getThumbnails($invId) {
 $db = acmeConnect();
 $sql = "SELECT imgPath, invName FROM images JOIN inventory ON images.invId = inventory.invId WHERE images.invId = :invId AND imgName LIKE '%-tn%'";
 $stmt = $db->prepare($sql);
 $stmt->bindValue(':invId', $invId, PDO::PARAM_INT);
 $stmt->execute();
 $imageArray = $stmt->fetchAll(PDO::FETCH_ASSOC);
